#!/usr/bin/env python3
"""Generate a realistic Honda Smart Car Tracking CARLA dataset.
...
quality settings and adds a subtle CCTV-style post-process to the rendered MP4s.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the Honda Smart Car Tracking POC dataset with realistic CARLA/CCTV visuals."
    )
    parser.add_argument("--output-dir", default="datasets/carla_honda_poc_realistic")
    parser.add_argument("--docs-dir", default="docs/realistic")
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Apply subtle CCTV color, grain, vignette, scanline, and REC timestamp styling.",
    )
    parser.add_argument(
        "--cctv-grain-strength",
        type=float,
...
def write_camera_graph(path: Path) -> None:
    graph = {
        "dataset": "carla_honda_poc_realistic",
        "renderer": "storyboard",
...
def cached_vignette(width: int, height: int) -> np.ndarray:
    ...
    if key not in _VIGNETTE_CACHE:
        ...
        cx = width * 0.5
        cy = height * 0.52
        distance = np.sqrt(((x - cx) / (width * 0.72)) ** 2 + ((y - cy) / (height * 0.72)) ** 2)
        vignette = 1.0 - np.clip((distance - 0.18) * 0.34, 0.0, 0.20)
...
def draw_cctv_overlay(frame: np.ndarray, camera_id: str, timestamp_sec: float, frame_id: int) -> None:
    ...
    cv2.putText(
        frame,
        f"REC  {camera_id}  T+{timestamp_sec:05.1f}s",
        (58, 45),
...
def apply_realistic_cctv_postprocess(
    frame: np.ndarray,
    camera_id: str,
    timestamp_sec: float,
    frame_id: int,
    ...
    # Mild CCTV grading: more local contrast, cooler shadows, softer highlights.
    graded = (graded - 128.0) * 1.08 + 128.0
    graded[:, :, 0] += 5.0
    graded[:, :, 1] += 1.5
    graded[:, :, 2] -= 3.0
    gray = cv2.cvtColor(np.clip(graded, 0, 255).astype(np.uint8), cv2.COLOR_BGR2GRAY).astype(np.float32)
    graded = gray[:, :, None] * 0.08 + graded * 0.92

    blurred = cv2.GaussianBlur(np.clip(graded, 0, 255).astype(np.uint8), (0, 0), 1.0).astype(np.float32)
    graded = cv2.addWeighted(graded, 1.08, blurred, -0.08, 0.0)
    ...
    draw_cctv_overlay(output, camera_id, timestamp_sec, frame_id)
    return output
...
def spawn_carla_cameras(
    ...
    for attr, value in {
        ...
        "exposure_compensation": "-0.65",
        "gamma": "2.05",
        "motion_blur_intensity": "0.08",
        "lens_flare_intensity": "0.03",
        "bloom_intensity": "0.08",
        "chromatic_aberration_intensity": "0.10",
...
def write_carla_camera_graph(
    ...
    graph = {
        ...
        "renderer": "carla",
        "visual_profile": {
            "preset": "realistic_cctv",
            "weather": "low sun, mild cloud, light atmospheric fog",
            "postprocess": "contrast, cool CCTV grade, vignette, scanlines, deterministic grain",
            "quality_note": "For stronger real-time shadows, start CARLA with -quality-level=Epic or High.",
        },
        "carla_map": carla_map,
...
def render_carla(
    ...
    log_step("Realistic visual profile enabled. For best shadows, run CARLA with -quality-level=Epic or High.")
                ...
                try:
                    ...
                    for local_frame_id in range(frame_count):
                        ...
                        for car in cars:
                            ...
                            bbox_fh.write(json.dumps(record, ensure_ascii=False) + "\n")
                        if cctv_postprocess:
                            frame = apply_realistic_cctv_postprocess(
                                frame,
                                camera.camera_id,
                                timestamp_sec,
                                local_frame_id,
...
Run CARLA 0.9.15 first, then generate the realistic 3D CCTV dataset:

```bash
python scripts/generate_carla_dataset_realistic.py --clean --write-contact-sheets
```

For metadata-only development without CARLA, use the storyboard dry-run:

```bash
python scripts/generate_carla_dataset_realistic.py --renderer storyboard --clean
```
"""
...
def main() -> int:
    ...
    if args.renderer == "carla":
        ...
        print(f"Generated realistic CARLA 3D CCTV videos with CARLA {carla_version}.")
    else:
        ...
        print("Generated storyboard dry-run videos. Use --renderer carla for the realistic 3D CCTV dataset.")
